    ...
    internal class WebSocketConnectionRfc6455 :IDisposable
    {
        ...
        internal void Close(WebSocketCloseReasons reason)
        {
            try
            {
                switch (reason)
                {
                    case WebSocketCloseReasons.ProtocolError:
                    case WebSocketCloseReasons.UnexpectedCondition:
                        {
                            //LogManager.Information($"{this.GetType().FullName} Error reading/writing to socket connecton, probably due to client crash.{Environment.NewLine}" +
                            //    $"Reason: {Enum.GetName(typeof(WebSocketCloseReasons), reason)}. Closing connection");
                            break;
                        }
                    case WebSocketCloseReasons.NormalClose:
                        {
                           // LogManager.Information($"{this.GetType().FullName} Normal close of socket connection.");
                            break;
                        }
                    default:
                        {
                            //LogManager.Information($"{this.GetType().FullName} Unexpected reason for closing socket connection.{Environment.NewLine}" +
                            //    $"Reason: {Enum.GetName(typeof(WebSocketCloseReasons), reason)}. Closing connection");
                            break;
                        }
                } 
            ...
            {
                //LogManager.Debug($"{this.GetType().FullName}.{nameof(Close)} Ignored exception caught. Exception details: {ex}");
            }
        }
        private void Dispose(Boolean disposing)
        {
            if (!_isDisposed)
            {
                ...
                {
                    //LogManager.Debug($"{this.GetType().FullName}.{nameof(Dispose)} Ignored exception caught. Exception details: {ex}");
                }
                finally
                {
